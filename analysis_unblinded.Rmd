...
woman_long %>%
  filter(measure == "panas") %>%
    ggplot(aes(x = time, y = value, color = factor(condition))) +
    geom_line(stat = "summary", fun = mean) +
    geom_point(stat = "summary", fun = mean) +
    labs(title = "Subjective Arousal Over Time",
       x = "Time",
...
emmeans(man_check_sa, pairwise ~ condition * time)
...
emmeans(man_check_pa, pairwise ~ condition * time)
