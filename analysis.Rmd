...
```{r}
# No missing values in condition
...
media_blind %>%
  filter(gender == "Female", cycle_regular == "Yes", contracept == "No") %>%
  count()
...
model_panas_3 <-
  woman_long |>
  filter(measure == "panas") |>
  lmer(value ~ condition * contracept * cycle_phase5 +
         condition * time * cycle_phase5 +
         caffeine_perday + mf_baseline_mean + 
         (1 | PARTICIPANT_ID), 
       data = _)
...
model_eda_3 <-
  woman_long |>
  filter(measure == "eda") |>
  lmer(value ~ condition * cycle_phase5 * contracept +
         condition * time +
         cycle_phase5 * time +
         caffeine_perday + mf_baseline_mean +
         (1 | PARTICIPANT_ID),
...
model_vs_1 <-
  ...
  lmer(value ~ condition * contracept *
         time + caffeine_perday + mf_baseline_mean +
         (1 | PARTICIPANT_ID),
       data = _)
summary(model_vs_1)
confint(model_vs_1)
...
model_vschange_3 <-
  woman_long |>
  lm(vs_change ~ condition * contracept * cycle_phase5 +
        caffeine_perday + mf_baseline_mean,
      data = _)
